...
server <- function(input, output, session){
  ...
  observeEvent(input$compare_journeys,{
    ...
    output$map_sliders <- renderUI({
      fluidRow(
        column(4, align='center', sliderInput(inputId = "max_stay_map", label = HTML('<img src= "clock_32.png"><br /><br /><span class ="notbold">Length of stay in minutes</span>'), min = 30, max=240, value = input$length_of_stay, step=30)),   bsTooltip("max_stay_map", "Increase the time you will be staying at the CBD and see how the parkings that are not within your time limits turn yellow. Please not that it ONLY has an effect on the map and not on the below calculations.", placement = "top", trigger = "hover",options = NULL),
